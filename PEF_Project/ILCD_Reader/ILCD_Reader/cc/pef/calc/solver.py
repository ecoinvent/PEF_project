...
def store_LCIA(data, path):
    ...
    df_lcia = df_lcia.rename(columns=C_idx.set_index('index')['indicator'].to_dict())
    df_lcia = A_idx[["index", "activityName", "geography", "product"]].join(df_lcia).drop(["index"], axis=1)
